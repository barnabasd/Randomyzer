...
public class ItemFiltering_Old {
    ...
    public static @NotNull LiteralArgumentBuilder<CommandSourceStack> CreateCommand() {
        ...
        command.then(addPart).then(rmPart)
            .then(Commands.literal("reset").executes(ItemFiltering_Old::Reset))
            .then(Commands.literal("list").executes(ItemFiltering_Old::ListMods)
        );
        return command;
    }
